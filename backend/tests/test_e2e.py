...
class TestE2EAuthRejection:
    ...
    async def test_projects_no_auth_401(self, client: AsyncClient):
        resp = await client.get("/api/projects")
        assert resp.status_code == 401

    async def test_projects_invalid_token_401(self, client: AsyncClient):
...
class TestE2EEntityRelationships:
    ...
    async def test_project_sessions(self, client: AsyncClient):
        """Create project -> session, then list through parents."""
