...
from codehive.api.schemas.issue import IssueCreate, IssueRead, IssueReadWithSessions, IssueUpdate
...
# Flat routes (get, update, delete, link-session)
...
async def list_issues_endpoint(
    ...
    try:
        issues = await list_issues(db, project_id, status=status)
    except ProjectNotFoundError:
        raise HTTPException(status_code=404, detail="Project not found")
