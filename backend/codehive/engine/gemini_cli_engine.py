"""Gemini CLI engine adapter: fire-and-forget subprocess per message with --resume."""
...
class _SessionState:
    """Internal per-session state for a Gemini CLI engine session."""

    def __init__(self) -> None:
        self.gemini_session_id: str | None = None
        self.retry_count: int = 0
        self.paused: bool = False
        self.pending_actions: dict[str, dict[str, Any]] = {}
        self.parser: GeminiCLIParser = GeminiCLIParser()


class GeminiCLIEngine:
    """Engine adapter using fire-and-forget Gemini CLI subprocesses.

    Each call to :meth:`send_message` spawns a new ``gemini -p`` subprocess.
    The first message starts a fresh session; subsequent messages use
    ``--resume <sessionId>`` to maintain conversation context.

    The ``gemini_session_id`` is captured from the ``init`` event (unlike
    Copilot which captures it from ``result``).
    ...
    async def send_message(
        ...
        **kwargs: Any,
        ...
        """Send a message via a new ``gemini -p`` subprocess and yield events.

        Spawns a subprocess per invocation.  On the first message for a session,
        no ``--resume`` flag is used; the ``gemini_session_id`` is captured from
        the ``init`` event.  Subsequent messages use ``--resume <sessionId>``.
        ...
        try:
            async for line in process.run(
                ...
                events = state.parser.parse_line(line, session_id)
                for event in events:
                    # Capture gemini_session_id from init event
                    if event.get("type") == "session.started":
                        gemini_sid = event.get("gemini_session_id", "")
                        if gemini_sid:
                            state.gemini_session_id = gemini_sid
                    yield event
            # Success -- reset retry count
            state.retry_count = 0
        except GeminiProcessError as exc:
            logger.warning(
                "Gemini process crashed for session %s (exit code %d): %s",
                session_id,
                exc.exit_code,
                exc.stderr[:200],
            )
    ...
    async def _retry_loop(
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            if not state.gemini_session_id:
                yield {
                    "type": "session.failed",
                    "session_id": str(session_id),
                    "error": "Process crashed and no gemini session ID available for resume",
                }
                return
            ...
            logger.info(
                ...
                MAX_RETRIES,
                session_id,
                state.gemini_session_id[:8],
            )

            retry_process = GeminiCLIProcess(
                cli_path=self._cli_path,
                working_dir=self._working_dir,
                extra_flags=self._extra_flags,
            )
